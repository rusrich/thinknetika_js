...
// Добавляю глобальную переменную для подсчета количества,
// прошедших регистрацию
let registeredSeats = 0

function eRegistration(ticket, fullName, nowTime) {
    const flightTickets = flights.BH118
    let checkTicket = flightTickets.tickets.find(item => item.id === ticket.id)
        
    if (!checkTicket) {
        throw new Error('No find ticket')
    } else {
        // console.log(true)
    }
    
    let checkPassenger = flightTickets.tickets.find(item => item.fullName === fullName)
    
    if (!checkPassenger) {
        throw new Error('No find passenger')
    } else {
        // console.log(true)
    }

    if (nowTime < flightTickets.registrationStarts) {
        throw new Error('Registration is not yet available')
    } else if (nowTime > flightTickets.registartionEnds) {
        throw new Error('Registration expiration')
    } else {
...
const b = eRegistration(a, 'Ivanov I. I.', makeTime(11, 20))
// console.log(b)
...
function flightReport(flightCode, nowTime) {
    const flight = flights[flightCode]
    let registration
    let complete
    ...
    return `
    Номер рейса: ${flight}
    Регистрация доступна: ${registration}
    Регистрация завершена: ${complete}
    Общее количество мест: ${countOfSeats}
    Количество купленных мест: ${reservedSeats}
    Количество зарегистрированных пассажиров: ${registeredSeats}
    `
    
}

const c = flightReport('BH118', makeTime(16, 0))
// console.log(c)
...
function flightDetails(flightName) {
    ...
    let divData = document.getElementById('details')
    
    for (let key in dataArray) {
        let reportInfo = document.createElement('p')
        divData.append(reportInfo)
        reportInfo.append(key, dataArray[key])
    }
    ...
    for (tick of allTickets) {
        const li = document.createElement('li')
        ul.append(li)

        const liText = document.createTextNode(`${tick.id} | ${tick.flight} | ${tick.fullName}`)
        li.append(liText)
        li.style.listStyle
    }
}

const d = flightDetails('BH118')
console.log(d)
