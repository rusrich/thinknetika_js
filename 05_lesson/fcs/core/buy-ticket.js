'use strict';

/**
 * Поиск свободного места нужного типа
 *
 * Гарантирует что найдет свободное место нужного типа или вернет null
 *
 * @param {Flight} flight
 * @param {number} type
 * @returns {number} seat
 */
function findAvailableSeat(flight, type) {
    let exists;
    let seat;
    let seatsOfType = 0;

    switch (type) {
        case 0: // standart
            const availableSeats = [];

            for (let i = flight.businessSeats + 1; i <= flight.seats; i++)
                if (!flight.tickets.find(item => item.seat === i))
                    availableSeats.push(i);

            if (availableSeats.length === 0)
                return null;

            const index = Math.floor(Math.random() * availableSeats.length);
            return availableSeats[index];
        case 1: // business
            for (let i = 1; i <= flight.businessSeats; i++)
                if (!flight.tickets.find(item => item.seat === i))
                    seatsOfType++;

            if (seatsOfType === 0)
                return null;

            do {
                seat = Math.floor(Math.random() * flight.businessSeats) + 1;
                exists = flight.tickets.find(item => item.seat === seat);
            } while (exists);

            return seat;
        default:
            throw new Error(`Unknown type`);
    }
}

/**
 * Покупка билета на самолет
 *
 * * проверка рейса
 * * проверка возможности купить (время и наличие мест)
 * * сохранение данных билета в информации о рейсе
 *
 * @param {World.flights} flights Все рейсы
 * @param {string} flightName Номер рейса
 * @param {number} buyTime Время покупки
 * @param {string} fullName Имя пассажира
 * @param {number} type Тип места
 * @returns { {world: World, ticket: Ticket} } Возвращаем копию билета
 */

function buyTicket(flights, flightName, buyTime, fullName, type = 0) {
    /**
     * @type {Flight}
     */


    const flight = flights[flightName];

    if (!flight)
        throw new Error('Flight not found');

    if (flight.tickets.length >= flight.seats)
        throw new Error('No seats available');

    if (buyTime > flight.registartionEnds)
        throw new Error('Time away');

    const seat = findAvailableSeat(flight, type);
    if (!seat)
        throw new Error(`No seats of type ${type} available. You can choose another type`);

    let id, exists;
    do {
        id = flight.name + '-' + (flight.tickets.length + 1); // Math.random().toString().substr(2, 3);
        exists = flight.tickets.find(item => item.id === id);
    } while (exists);

    /**
     * @type {Ticket}
     */
    const ticket = {
        id,
        flight: flight.name,
        buyTime,
        fullName,
        registrationTime: null,
        type,
        seat,
    };

    flight.tickets.push(ticket);

    return {...ticket}; // копируем, что бы "пассажир" случайно ничего не изменил в самой информации о рейсе
}


let flightNameInput, fullNameInput, buyTimeInput

const btnBuyTicket = document.getElementById('buyticketsubmit')
btnBuyTicket.addEventListener('click', getDataforBuyTicket)

function getDataforBuyTicket(event) {
    event.preventDefault()
    flightNameInput = document.getElementsByTagName('input')[2].value
    if (flightNameInput !== 'BH118') {
        const reportErr = document.getElementById('reporterr')
        const paragrapherr = document.createElement('p')
        paragrapherr.innerText = 'Flight didn\'t find'
        reportErr.append(paragrapherr)
        reportErr.style.display = 'block'
        throw new Error (`Flight didn't find`)
    } else {
        fullNameInput = document.getElementsByTagName('input')[3].value
        console.log(flightNameInput, fullNameInput);
        
        let res3 = buyTicket(world.flights, flightNameInput, makeTime(5, 10), fullNameInput);
        updateView();
    }
        


}