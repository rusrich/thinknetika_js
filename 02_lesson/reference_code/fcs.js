...
function flightReport(flight, nowTime) {
    const flight2 = flights[flight]
    let registration
    let complete
    if (nowTime < flight2.registrationStarts && nowTime > flight2.registartionEnds) {
        registration = true
    } else {
        registration = false
    }

    if (nowTime > flight2.registartionEnds) {
        complete = true
    } else {
        complete = false
    }

    let countOfSeats = flight2.seats
    let reservedSeats = flight2.tickets.length
    console.log(`
    Номер рейса: ${flight}
